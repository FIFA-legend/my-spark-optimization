...
object JoinsRecap {
  ...
  // outer joins
  // left outer = everything in inner join + all the rows in the LEFT, with null in the rows not passing the condition in the RIGHT table
  guitaristsDF.join(bandsDF, joinCondition, "left_outer")

  // right outer = everything in inner join + all the rows in the RIGHT, with null in the rows not passing the condition in the LEFT table
  ...
  // dangerous: NRows(cross_join) = NRows(left) x NRows(right)
