...
object RDDSkewedJoins {
  ...
  /*
    ...
    Acer Predator 2.9Ghz ayfaskjhrw -> average sale price of all Acer Predators with CPU speed between 2.8 and 3.0 GHz
   */
  ...
  def plainJoin() = {
    ...
    val result = preparedLaptops.join(preparedOffers) // RDD[((make, model), ((registration, procSpeed), (procSpeed, salePrice)))]
      .filter {
        case ((_, _), ((_, laptopCpu), (offerCpu, _))) => Math.abs(laptopCpu - offerCpu) <= 0.1
      }
      .map {
        case ((_, _), ((reg, _), (_, salePrice))) => (reg, salePrice)
      }
      .aggregateByKey((0.0, 0))(
        ...
        {
          case ((totalPrices1, numPrices1), (totalPrices2, numPrices2)) => (totalPrices1 + totalPrices2, numPrices1 + numPrices2) // combine two states into one
        }
